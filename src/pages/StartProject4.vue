    ...
    <div>
      <transition name="slide-fade">
        <div v-if="show" class="card timer">
            <h2>{{this.$store.state.time4[2].hour}} : {{this.$store.state.time4[1].minut}} : {{this.$store.state.time4[0].second}}</h2>
            <div>
...
export default {
  data() {
    return {
      ...
      disable: false,
      userProject: {
        title: "" || localStorage.getItem('project4_title'),
        username: '' || localStorage.getItem('project4_username'),
        summa: 0,
        end: false,
        pause: 0 || localStorage.getItem('project4_pause')
      }
    };
  },
  created () {
  ...
  methods: {
    finishProject ( ) {
      ...
      const data = {
        ...
        'start': localStorage.getItem('project4_startTime')
      }
      this.show = false
      this.disable = false
    ...
    startTimer ( ) {
      localStorage.setItem('project4', 1)
      localStorage.setItem('project4_title', this.userProject.title)
      localStorage.setItem('project4_username', this.userProject.username)
      this.show = true
      this.disable = true
      // save started time
      var today = new Date();
      ...
      var time = today.getHours() + ":" + today.getMinutes() + ":" + today.getSeconds();
      var dateTime = date+' '+time;
      localStorage.setItem('project4_startTime', dateTime)
      // 
    ...
    timer ( ) {
      this.$store.commit('setTime4')
    },
    pause ( ) {
      this.toggle = false
      localStorage.setItem('project4_pause', 1)
      this.userProject.pause = '1'
    },
    resume ( ) {
      this.toggle = true
      localStorage.setItem('project4_pause', 0)
      this.userProject.pause = '0'
    },
    calculating ( ) {
      this.userProject.summa = (this.$store.state.time4[1].minut/60 + this.$store.state.time4[2].hour) * this.$store.state.admin.perHour
    }
  }
};
</script>
