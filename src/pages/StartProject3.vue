    ...
    <div>
      <transition name="slide-fade">
        <div v-if="show" class="card timer">
            <h2>{{this.$store.state.time3[2].hour}} : {{this.$store.state.time3[1].minut}} : {{this.$store.state.time3[0].second}}</h2>
            <div>
...
export default {
  data() {
    return {
      ...
      disable: false,
      userProject: {
        title: "" || localStorage.getItem('project3_title'),
        username: '' || localStorage.getItem('project3_username'),
        summa: 0,
        end: false,
        pause: 0 || localStorage.getItem('project3_pause')
      }
    };
  },
  created () {
  ...
  methods: {
    finishProject ( ) {
      ...
      const data = {
        ...
        'start': localStorage.getItem('project3_startTime')
      }
      this.show = false
      this.disable = false
    ...
    startTimer ( ) {
      localStorage.setItem('project3', 1)
      localStorage.setItem('project3_title', this.userProject.title)
      localStorage.setItem('project3_username', this.userProject.username)
      this.show = true
      this.disable = true
      // save started time
      var today = new Date();
      ...
      var time = today.getHours() + ":" + today.getMinutes() + ":" + today.getSeconds();
      var dateTime = date+' '+time;
      localStorage.setItem('project3_startTime', dateTime)
      // 
    ...
    timer ( ) {
      this.$store.commit('setTime3')
    },
    pause ( ) {
      this.toggle = false
      localStorage.setItem('project3_pause', 1)
      this.userProject.pause = '1'
    },
    resume ( ) {
      this.toggle = true
      localStorage.setItem('project3_pause', 0)
      this.userProject.pause = '0'
    },
    calculating ( ) {
      this.userProject.summa = (this.$store.state.time3[1].minut/60 + this.$store.state.time3[2].hour) * this.$store.state.admin.perHour
    }
  }
};
</script>
